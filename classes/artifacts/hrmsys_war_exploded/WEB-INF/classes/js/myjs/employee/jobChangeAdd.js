...
var JobChangeAddPanel = Ext.extend(Ext.form.FormPanel,{
	...
	constructor: function(){
		...
		var oldDeptJC = new DepartJob("旧门店","jobChange.departmentByJcOldDept.deptId");
		var oldJobJC = new Job("旧职位","jobChange.jobByJcOldJob.jobId", oldDeptJC);
		oldJobJC.on('expand', function(comboBox){
			var deptId = Ext.getCmp("deptValue旧门店").getValue();
			this.getStore().load({
				params: {
					deptId: deptId
				}
		...
		var newDeptJC = new DepartJob("新门店","jobChange.departmentByJcNewDept.deptId");
		var newJobJC = new Job("新职位","jobChange.jobByJcNewJob.jobId", newDeptJC);
		newJobJC.on('expand', function(comboBox){
			var deptId = Ext.getCmp("deptValue新门店").getValue();
			this.getStore().load({
				params: {
					deptId: deptId
				}
